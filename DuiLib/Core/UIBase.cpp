...
void CWindowWnd::SetPosAtCornerWindow(int type, int dx, int dy)
{
    ASSERT(::IsWindow(m_hWnd));
    ASSERT((GetWindowStyle(m_hWnd)&WS_CHILD)==0);
    RECT rcDlg = { 0 };
    ...
    // Find dialog's upper left based on rcCenter
    int xLeft = (rcCenter.left + rcCenter.right) - DlgWidth - dx;
    int yTop = (rcCenter.top + rcCenter.bottom) - DlgHeight - dy;

    // The dialog is outside the screen, move it inside
	if (type == 1)
	{
		xLeft = (rcCenter.left) + dx;
		yTop = (rcCenter.top) + dy;
	}
	else if (type == 2)
	{
		xLeft = (rcCenter.left + rcCenter.right) - DlgWidth - dx;
		yTop = (rcCenter.top) + dy;
	}
	else if (type == 3)
	{
		xLeft = (rcCenter.left) + dx;
		yTop = (rcCenter.top + rcCenter.bottom) - DlgHeight - dy;
	}
	else if (type == 4)
	{
		xLeft = (rcCenter.left + rcCenter.right) - DlgWidth - dx;
